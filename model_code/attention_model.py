...
class TransformerBlock(tf.keras.Model):
  ...
  def __init__(
    ...
    self.mlp_linear1 = tf.keras.layers.Dense(channels * 2)
    self.mlp_dropout1 = tf.keras.layers.Dropout(dropout_rate)
    self.mlp_linear2 = tf.keras.layers.Dense(channels)
    self.mlp_dropout2 = tf.keras.layers.Dropout(dropout_rate)
...
class MultiheadAttention(tf.keras.Model):
  ...
  def __call__(self,
    ...
    q = self._q_layer(inputs)  # [B, H, T, K]
    k = self._k_layer(inputs)  # [B, H, T, K]
    v = self._v_layer(inputs)  # [B, H, T, V]
    # print(q.shape)

    q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = self._num_heads), (q, k, v))
    # print(q.shape)
    ...
    if self._relative_positions:
      ...
      r_k = rearrange(r_k, 'b n (h d) -> b h n d', h = self._num_heads)
    ...
    attended_inputs = rearrange(output_transpose, 'b t h v -> b t (h v)')
